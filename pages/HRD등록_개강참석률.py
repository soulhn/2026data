...
from utils import check_password, page_error_boundary, calc_recruit_rate, load_data
...
st.set_page_config(page_title="HRD 등록 대비 승인률", page_icon="🎯", layout="wide")
check_password()
with page_error_boundary():
    st.title("🎯 HRD 등록 대비 승인률")
    st.markdown(
        "HRD-Net 집계 기준 **정원 → 수강신청(HRD 등록) → 승인 인원(기관 승인 = 확정 신고) → 수료** 퍼널을 "
    ...
    df['미승인'] = df['TOT_TRP_CNT'] - df['TOT_PAR_MKS']

    def _pct(numer, denom):
        return (numer / denom.replace(0, pd.NA) * 100).astype('Float64').round(1)

    df['등록 대비 승인률'] = _pct(df['TOT_PAR_MKS'], df['TOT_TRP_CNT'])
    ...
    # 승인 인원이 0인 개설예정 회차는 명부가 비므로 호출하지 않는다.
    ...
    # 명부를 못 읽은 종료 회차는 승인 인원 − 수료로 대신한다. HRD 수료(finiCnt)에 조기취업이 빠져 있어
    ...
    # ── 개강일 참석 (명부 사람 스냅샷 TB_ROSTER_MEMBER, kpi_etl이 매 실행 갱신) ──
    # 참석 판정 = 개강일에 입실 시간이 있거나 출석 계열 상태. 출결을 읽지 않은 회차(추적 전 종료)는 비워 둔다.
    try:
        day1 = load_data(
            "SELECT TRPR_ID, TRPR_DEGR, "
            "SUM(CASE WHEN FIRST_ATTEND_DT = REPLACE(TR_STA_DT, '-', '') THEN 1 ELSE 0 END) AS DAY1_CNT, "
            "SUM(CASE WHEN FIRST_ATTEND_DT IS NOT NULL THEN 1 ELSE 0 END) AS ATTENDED_CNT "
            "FROM TB_ROSTER_MEMBER GROUP BY TRPR_ID, TRPR_DEGR"
        )
        day1['TRPR_DEGR'] = pd.to_numeric(day1['TRPR_DEGR'], errors='coerce').fillna(0).astype(int)
        df = df.merge(day1, on=['TRPR_ID', 'TRPR_DEGR'], how='left')
    except Exception:
        df['DAY1_CNT'] = pd.NA
        df['ATTENDED_CNT'] = pd.NA
    df['개강일 참석'] = df['DAY1_CNT'].astype('Float64').astype('Int64')
    df.loc[df['ATTENDED_CNT'].astype('Float64').fillna(0) == 0, '개강일 참석'] = pd.NA
    df['개강일 참석률'] = (df['개강일 참석'].astype('Float64')
                      / df['TOT_PAR_MKS'].astype('Float64').replace(0, pd.NA) * 100).round(1)

    if roster_error:
        st.warning(f"일부 회차 명부 조회 실패 — 해당 회차의 이탈 인원·80%이상수료는 비어 있습니다: {roster_error}")
    elif roster_counts is None and data_source != "API":
        st.caption("명부 API를 쓸 수 없어 이탈 인원은 종료 회차만 승인 인원 − 수료로 표시합니다. 80%이상수료는 비어 있습니다.")
    ...
    st.caption("선택한 회차 합산 기준 · 등록 대비 승인률 = 승인 인원 / 수강신청 · 수료율은 종료 회차만 합산")
    ...
    k4.metric("승인 인원 합계", f"{tot_par:,}명", f"미승인 {tot_trp - tot_par:,}명", delta_color="off",
    ...
    day1_known = view[view['개강일 참석'].notna()]
    tot_day1 = int(day1_known['개강일 참석'].sum())
    par_of_day1 = int(day1_known['TOT_PAR_MKS'].sum())

    r1, r2, r3, r4, r5, r6 = st.columns(6)
    r6.metric("개강일 참석률", f"{tot_day1 / par_of_day1 * 100:.1f}%" if par_of_day1 else "-",
              f"참석 {tot_day1:,}명", delta_color="off",
              help="개강일 참석 / 승인 인원 × 100 — 명부 스냅샷에 출결이 있는 회차만 합산")
    r1.metric("등록 대비 승인률", f"{tot_par / tot_trp * 100:.1f}%" if tot_trp else "-",
              help="승인 인원 / 수강신청 × 100")
    r5.metric("이탈률", f"{tot_drop / par_of_known * 100:.1f}%" if par_of_known else "-",
              f"이탈 {tot_drop:,}명", delta_color="inverse",
              help="이탈 인원 / 승인 인원 × 100 — 이탈 인원이 확인된 회차만 합산")
    ...
    with c_right:
        ...
        if trend.empty:
            ...
            bars = base.mark_bar().encode(
                y=alt.Y('등록 대비 승인률:Q', axis=alt.Axis(title=['승', '인', '률', '(%)'], titleAngle=0),
                        scale=alt.Scale(domain=[0, 100])),
                color=alt.Color('과정:N', title='과정'),
                tooltip=['과정', '회차', '상태', 'TOT_TRP_CNT', 'TOT_PAR_MKS', '등록 대비 승인률'],
            )
            ...
            st.caption("점선 = 선택 회차 합산 등록 대비 승인률")
    ...
    st.caption(
        ...
        "조기취업은 HRD 수료(finiCnt)에 들어가지 않아 따로 표시합니다 (종료 회차: 승인 인원 = 이탈 + 수료 + 조기취업). "
        "표 우측 상단에서 CSV로 내려받아 회사 페이지와 대조할 수 있습니다."
    )

    table_cols = ['과정', '회차', '상태', 'TR_STA_DT', 'TR_END_DT',
                  'TOT_FXNUM', 'TOT_TRP_CNT', 'TOT_PAR_MKS', '미승인', '이탈 인원', 'FINI_CNT', '80%이상수료', '조기취업',
                  '등록 대비 승인률', '모집률', '정원 충원율', '개강일 참석', '개강일 참석률', '수료율', '이탈률']
    st.dataframe(
        ...
        column_config={
            ...
            "TOT_PAR_MKS": st.column_config.NumberColumn("승인 인원", format="%d명", help="기관이 HRD-Net에서 승인한 인원 = 확정 신고 인원 = 명부 인원. 개강 전에도 잡힘"),
            "미승인":    st.column_config.NumberColumn("미승인", format="%d명", help="수강신청 − 승인 인원: 신청만 하고 승인되지 않았거나 취소된 사람"),
            "이탈 인원":    st.column_config.NumberColumn("이탈 인원", format="%d명",
                                                       help="명부 상태 중도탈락·제적 (개강 후 확정 인원 기준). 명부를 못 읽은 종료 회차는 승인 인원 − 수료 (조기취업이 섞여 실제보다 클 수 있음)"),
            ...
            "등록 대비 승인률":  st.column_config.ProgressColumn("등록 대비 승인률(%)", format="%.1f%%", min_value=0, max_value=100),
            ...
            "개강일 참석":  st.column_config.NumberColumn("개강일 참석", format="%d명",
                                                       help="명부 스냅샷에서 첫 참석일이 개강일인 사람 (입실 시간 있거나 출석 계열 상태). 출결을 읽지 않은 회차는 비어 있음"),
            "개강일 참석률": st.column_config.ProgressColumn("개강일 참석률(%)", format="%.1f%%", min_value=0, max_value=100,
                                                          help="개강일 참석 / 승인 인원 × 100 — 승인된 사람 중 개강일에 실제로 온 비율"),
...
| 승인 인원 | `totParMks` | 확정 신고 인원으로 해석. **훈련생 명부(API 3) 건수와 정확히 일치**하는 것을 실측으로 확인 |
...
| 조기취업 | 명부 API `trneeSttusNm` | `조기취업` 상태 인원. HRD 수료(`finiCnt`)에 들어가지 않고 이탈도 아닌 별도 결과. 이 컬럼이 있어야 종료 회차의 승인 인원 = 이탈 + 수료 + 조기취업 이 맞아떨어짐 |
...
- **명부 조회 실패 회차** — 이탈 인원·80%이상수료가 비어 보입니다(상단 경고). 종료 회차만 승인 인원 − 수료로 이탈 인원을 대신 채우는데, 조기취업자가 섞여 실제 이탈보다 클 수 있습니다.
- **DB 폴백 시 엔코아 과정** — ETL이 한화 과정만 수집하므로 API 실패 시 한화만 보입니다 (상단 경고로 표시).

**가공한 것**

- 개설예정 회차의 승인 인원이 `None`이면 0으로 표시했습니다.
...
|---|---|---|---|---|
| 1 | 확정자 수 | 승인 인원 | 확정자 신고 인원 | 회차별로 같아야 정상. 다르면 `totParMks`가 "확정 신고"가 아니라 다른 단계(기관 승인 등)를 세는 것 |
...
| 8 | 80%이상수료 | 80%이상수료 | 80%수료(비용O) | 건수가 같아야 정상. 다르면 상태 반영 시점 차이 |
        ...
        if not pre_open.empty:
            rows = ", ".join(
                f"{r['과정']} {r['회차']} (개강 {r['TR_STA_DT']}, 신청 {int(r['TOT_TRP_CNT'])} / 승인 {int(r['TOT_PAR_MKS'])})"
                for _, r in pre_open.iterrows()
            )
            st.info(
                f"**개강 전 승인 진행 중인 회차** — {rows}. 승인 인원은 기관이 HRD-Net에서 승인한 시점에 잡히므로 "
                "개강 전에도 값이 있는 것이 정상입니다 (2026-09-15 실측: 승인자는 개강 전에도 명부에 훈련중으로 존재)."
            )
        else:
            st.info("개강 전 회차 중 승인이 시작된 곳은 현재 없습니다.")
...
| # | 결정 항목 | 선택지 | 기본값(현재) |
|---|---|---|---|
| 1 | `totParMks` 컬럼 이름 | ~~승인 인원~~ → **승인 인원** (2026-09-16 변경 완료. 근거: 노션 확정자신고·명부 인원과 일치, 개강 전에도 값 존재) | 승인 인원 |
| 2 | `totTrpCnt` 해석 | 누적 신청(취소 포함) / 유효 신청 | 누적 신청으로 표기 |
| 3 | 개설예정 회차 | 표에 포함 / 기본 필터에서 제외 | 포함 |
| 4 | 등록 대비 승인률 분모 | 수강신청 / 합격자 수(회사 데이터 수기 입력) | 수강신청 |
| 7 | 개강일 참석 | 명부 사람 스냅샷(TB_ROSTER_MEMBER)의 첫 참석일 = 개강일 (입실 시간 있거나 출석 계열 상태) | 2026-09-16 추가 |
