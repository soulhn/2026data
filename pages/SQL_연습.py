...
EXAMPLES = {
    ...
    "중급": [
        ("과정별 수료율 (JOIN + GROUP BY)",
         ...
         "       ROUND(SUM(CASE WHEN t.TRNEE_STATUS LIKE '%수료%'\n"
         "                       OR t.TRNEE_STATUS LIKE '%조기취업%'\n"
         "                  THEN 1.0 ELSE 0 END) * 100.0\n"
         "             / COUNT(1), 1) AS completion_rate\n"
        ...
        ("지역별 평균 모집률",
         ...
         "              ELSE NULL END), 1) AS avg_recruit_rate\n"
         "FROM TB_MARKET_TREND\n"
         "WHERE REGION IS NOT NULL AND REGION != ''\n"
         "GROUP BY REGION\n"
         "HAVING course_cnt >= 10\n"
    ...
    "고급": [
        ("과정별 출석률 (NOT_ATTEND 제외)",
         ...
         "                      THEN 1 ELSE 0 END), 0), 1) AS att_rate\n"
        ...
        ("CTE: 기관별 과정 수 & 평균 만족도",
         ...
         "  SELECT TRAINST_NM,\n"
         "         COUNT(*) AS course_cnt,\n"
         "         ROUND(AVG(STDG_SCOR), 2) AS avg_score\n"
         "  FROM TB_MARKET_TREND\n"
