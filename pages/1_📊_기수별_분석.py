...
def get_course_list():
    ...
    """
    return load_data(query, params=[today])
...
def get_analysis_data(degr):
    ...
    # 1. 과정 마스터 정보
    course_df = load_data("SELECT * FROM TB_COURSE_MASTER WHERE TRPR_DEGR = ?", params=[degr])
    
    # 2. 훈련생 정보 + 나이 계산
    trainee_df = load_data("SELECT * FROM TB_TRAINEE_INFO WHERE TRPR_DEGR = ?", params=[degr])
