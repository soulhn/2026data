...
def load_kpi_data(where, params):
    """KPI 섹션용 집계 쿼리."""
    if where == "":
        _c = get_market_cache(CacheKey.KPI)
        if _c is not None:
            return _c
    return _sql_query(f"""
        SELECT COUNT(*) as CNT,
...
def load_monthly_counts(where, params):
    """Tab 1, 6: 월별 개설 수."""
    if where == "":
        _c = get_market_cache(CacheKey.MONTHLY_COUNTS)
        if _c is not None:
            return _c
    return _sql_query(f"""
...
def load_region_counts(where, params):
    """Tab 1: 지역별 과정 수."""
    if where == "":
        _c = get_market_cache(CacheKey.REGION_COUNTS)
        if _c is not None:
            return _c
    return _sql_query(f"""
...
def load_inst_stats(where, params):
    """Tab 2, 9: 기관별 집계."""
    if where == "":
        _c = get_market_cache(CacheKey.INST_STATS)
        if _c is not None:
            return _c
    return _sql_query(f"""
        SELECT TRAINST_NM,
...
def load_ncs_agg(where, params, min_courses=5):
    ...
    if where == "" and min_courses == 5:
        _c = get_market_cache(CacheKey.NCS_AGG)
        if _c is not None:
            return _c
    return _sql_query(f"""
        SELECT NCS_CD,
...
def load_monthly_recruit(where, params):
    """Tab 7: 월별 평균 모집률."""
    if where == "":
        _c = get_market_cache(CacheKey.MONTHLY_RECRUIT)
        if _c is not None:
            return _c
    return _sql_query(f"""
        SELECT YEAR_MONTH,
...
def load_region_opp(where, params):
    ...
    if where == "":
        _c = get_market_cache(CacheKey.REGION_OPP)
        if _c is not None:
            return _c
    return _sql_query(f"""
        SELECT REGION,
...
with page_error_boundary():
    ...
    with tabs[0]:
        ...
        st.markdown("모집률 높고 과정수 적은 지역(🔵) = 공급 부족 → **신규 진입 기회**")
