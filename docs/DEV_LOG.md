...
DB TB_COURSE_MASTER → 한화(AIG...455635) 25행뿐, 최신 종료일 2026-07-03
                      엔코아 과정: 0행
```
- `hrd_etl.py`는 `HANWHA_COURSE_ID` **하나만** 수집한다. ETL 워크플로는 매시간 정상 실행 중(최근 5회 전부 success)이지만 수집 대상에 엔코아가 없다 → **DB 폴백은 현재 운영 기수에 대해 구조적으로 무의미**
- `get_institutions()` 기반으로 ETL을 확장하면 폴백이 살아나지만, 매출 분석·종료과정 성과 등 한화 기준으로 돌아가던 페이지에 엔코아 데이터가 섞여 지표가 바뀐다 → **별건으로 분리**하고 이번에는 ②의 정직한 안내로 대응
...
- **미수정으로 남긴 것**: `pages/매출_분석.py:471`이 `_출석` 컬럼에 출석 수(int)와 `"-"`(str)를 섞어 Arrow 직렬화 경고 발생. Streamlit이 자동 보정해 표는 정상 렌더되므로 표시 방식 결정이 필요한 별건
