...
### 인원 필드 해석 (2026-09-08 실측)

| 필드 | 의미 | 비고 |
|---|---|---|
| `totFxnum` | 정원 | |
| `totTrpCnt` | 수강신청 (HRD 등록) | 확정 여부와 무관한 누적 신청 수 |
| `totParMks` | 수강인원 (확정 신고) | **API 3 명부 건수와 정확히 일치** (25회차 30=30, 24회차 21=21). 개설예정 회차는 `None` |
| `finiCnt` | 수료 인원 = 정상수료 + 80%이상수료 | **조기취업 제외** (한화 3회차: 명부 정상 22 + 80% 1 = 23 = finiCnt, 조기취업 1 미포함). 앱의 수료 판정(`수료|조기취업`)과 기준이 달라 `totParMks − finiCnt`는 중도탈락 + 조기취업. 진행중 회차는 0 |

- 회차 레코드는 **현재 시점 스냅샷** — 상태 변경 일시 필드 없음. 신청→확정 전환 시점을 보려면 수집 시각과 함께 별도 이력 저장 필요
- 이 API는 기관 소유권 검사가 없어 한화 키로 엔코아 과정도 조회됨 (`hrd_api.fetch_course_history`)
- **회차 0 레코드**: 과정마다 1건, `trStaDt`/`trEndDt`가 `None`·`totTrpCnt` 0·`totFxnum`만 채워진 과정 헤더(템플릿) 행. 실제 기수가 아니므로 `_request_course_records`가 제외
