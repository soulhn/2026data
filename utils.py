...
def get_hrd_cache(key: str):
    """TB_MARKET_CACHE에서 단일 스칼라 값을 반환. 없으면 None."""
    import json
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(adapt_query("SELECT CACHE_DATA FROM TB_MARKET_CACHE WHERE CACHE_KEY = ?"), (key,))
        row = cur.fetchone()
        if row is None:
            return None
        val = row[0] if not isinstance(row, dict) else row['cache_data']
        return json.loads(val)
    except Exception:
        return None


def set_hrd_cache(key: str, value) -> bool:
    """TB_MARKET_CACHE에 단일 스칼라 값을 저장. 성공 시 True."""
    import json
    try:
        conn = get_connection()
        cur = conn.cursor()
